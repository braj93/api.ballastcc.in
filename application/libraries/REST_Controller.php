...
abstract class REST_Controller extends CI_Controller {
    ...
    public function _remap($object_called, $arguments = []) {
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && $this->_allow === FALSE) {
            ...
            // change HTTP_FORBIDDEN to HTTP_UNAUTHORIZED.
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
    ...
    protected function _detect_api_key() {
        ...
        $this->rest->active_profile_type = NULL;
        $this->rest->ignore_limits = FALSE;

        // Find the key from server or arguments
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) {
            if (!($row = $this->rest->db->where($this->config->item('rest_key_column'), $key)->where('status', 'ACTIVE')->get($this->config->item('rest_keys_table'))->row())) {
                return FALSE;
            }
            ...
            isset($row->active_profile_type) && $this->rest->active_profile_type = $row->active_profile_type;
            isset($row->level) && $this->rest->level = $row->level;
            isset($row->ignore_limits) && $this->rest->ignore_limits = $row->ignore_limits;

            $this->_apiuser = $row;

            $this->rest->db->update($this->config->item('rest_keys_table'), ["last_used_at" => DATETIME], [$this->config->item('rest_key_column') => $this->rest->key]);
            /*
    ...
    protected function _log_access_time() {
        ...
        if (property_exists($this->rest, 'db')) {
