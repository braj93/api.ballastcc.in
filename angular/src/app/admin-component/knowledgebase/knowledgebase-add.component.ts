...
@Component({
  selector: 'app-knowledgebase-add',
  templateUrl: './knowledgebase-add.component.html',
...
export class KnowledgebaseAddComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.userService.page_title = 'Add Knowledge Base';
    const is_upload = this.kbForm.get('is_upload');
    // console.log("is_upload", is_upload);
  ...
  submitForm() {
    ...
    this.adminService.createKnowledgebase(this.kbForm.value).subscribe((response: any) => {
