...
@Component({
  selector: 'fitness-template',
  templateUrl: './fitness-template.component.html',
  styleUrls: ['./fitness-template.component.css']
})
export class FitnessTemplateComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    if (this.details.template_values) {
      ...
      this.campaignService.fitness_values = parseValues; 
    }else if (this.details.default_values){
      this.campaignService.fitness_values = this.details.default_values;
    }
  ...
  updateTemplateDetails(){
    ...
    let data = {
      ...
      template_values : JSON.stringify(this.campaignService.fitness_values)
    }
  ...
  getTargetValue(target){
    ...
    if (target_array.length === 2) {
      value = this.campaignService.fitness_values[target_array[0]][target_array[1]];
    } else if(target_array.length === 3) {
      value = this.campaignService.fitness_values[target_array[0]][target_array[1]][target_array[2]];
    } else if(target_array.length === 4){
      value = this.campaignService.fitness_values[target_array[0]][target_array[1]][target_array[2]][target_array[3]];
    } else if(target_array.length === 5){
      value = this.campaignService.fitness_values[target_array[0]][target_array[1]][target_array[2]][target_array[3]][target_array[4]];
    }
  ...
  receiveBannerResponse($event: any) {
    this.media_object = $event.data;
    this.campaignService.fitness_values.banner_background_image = {
