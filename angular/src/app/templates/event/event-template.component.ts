...
export class EventTemplateComponent implements OnInit{
  ...
  constructor(
    ...
    ngOnInit(): void {
      ...
      if (this.details.template_values) {
        ...
        this.campaignService.events_values = parseValues; 
      }else if (this.details.default_values){
        this.campaignService.events_values = this.details.default_values;
      }
  ...
  updateTemplateDetails(){
    ...
    let data = {
      ...
      template_values : JSON.stringify(this.campaignService.events_values)
    }
  ...
  getTargetValue(target){
    ...
    if (target_array.length === 1) {
      value = this.campaignService.events_values[target_array[0]];
    } else if (target_array.length === 2) {
      value = this.campaignService.events_values[target_array[0]][target_array[1]];
    } else if(target_array.length === 3) {
      value = this.campaignService.events_values[target_array[0]][target_array[1]][target_array[2]];
    } else if(target_array.length === 4){
      value = this.campaignService.events_values[target_array[0]][target_array[1]][target_array[2]][target_array[3]];
    } else if(target_array.length === 5){
      value = this.campaignService.events_values[target_array[0]][target_array[1]][target_array[2]][target_array[3]][target_array[4]];
    }
  ...
  receiveBannerResponse($event: any) {
    ...
    values.title = this.media_object.original_name;
    // this.media_object = $event.data;
    // this.campaignService.events_values.banner_background_image = {
