import { Component, OnInit, Input, TemplateRef, ElementRef, ViewChild, OnDestroy } from '@angular/core';
...
@Component({
  selector: 'detailed-template',
  templateUrl: './detailed-template.component.html',
  styleUrls: ['./detailed-template.component.css']
})
export class DetailedTemplateComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (this.details.template_values) {
      ...
      this.campaignService.detailed_values = parseValues;
    } else if (this.details.default_values) {
      this.campaignService.detailed_values = this.details.default_values;
    }
  ...
  updateTemplateDetails(){
    ...
    let data = {
      ...
      template_values: JSON.stringify(this.campaignService.detailed_values)
    }
  ...
  getTargetValue(target) {
    ...
    if (target_array.length === 2) {
      value = this.campaignService.detailed_values[target_array[0]][target_array[1]];
    } else if (target_array.length === 3) {
      value = this.campaignService.detailed_values[target_array[0]][target_array[1]][target_array[2]];
    } else if (target_array.length === 4) {
      value = this.campaignService.detailed_values[target_array[0]][target_array[1]][target_array[2]][target_array[3]];
    } else if (target_array.length === 5) {
      value = this.campaignService.detailed_values[target_array[0]][target_array[1]][target_array[2]][target_array[3]][target_array[4]];
    }
  ...
  receiveBannerResponse($event: any) {
    this.media_object = $event.data;
    this.campaignService.detailed_values.banner_background_image = {
